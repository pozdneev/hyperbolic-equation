...
double U_basic(double x0, double x1, double x2, double t, Grid_Info g)
{
    ...
    /*
    if (fabs(x0 - 0.35) > g.L0/2) x0 -= g.L0;
    if (fabs(x1 - 0.25) > g.L1/2) x1 -= g.L1;
    if (fabs(x2 - 0.15) > g.L2/2) x2 -= g.L2;

    return exp(-(SQR(x0 - 0.35) + SQR(x1 - 0.25) + SQR(x2 - 0.15)) / 0.01);
    */
}
// --------------------------------------------------------------------------
        ...
        double *x[], double *y[], Grid_Info g, double tau, fun_t fun)
{
    ...
    for (i0 = 1; i0 <= g.Ns0-1; ++i0)
    {
        ...
        {
            for (i2 = 1; i2 <= g.Ns2-1; ++i2)
            {
                idx   = IDX3(i0,   i1,   i2,   0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
                idx0l = IDX3(i0-1, i1,   i2,   0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
                idx0r = IDX3(i0+1, i1,   i2,   0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
                idx1l = IDX3(i0,   i1-1, i2,   0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
                idx1r = IDX3(i0,   i1+1, i2,   0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
                idx2l = IDX3(i0,   i1,   i2-1, 0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
                idx2r = IDX3(i0,   i1,   i2+1, 0, 0, 0, g.Ns0+1, g.Ns1+1, g.Ns2+1);
...
int main()
{
    ...
    Grid_Info g = { 10, 10, 10, 1.0, 1.0, 1.0, -1, -1, -1 };

    const double T = 1*g.L0;
    const int K = 2*g.Ns0;
    const double tau = T / (K);   // TODO: what is stability condition?
