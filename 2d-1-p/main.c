...
//        cos(PI * t * sqrt(1 / SQR(g.L0) + 1 / SQR(g.L1)));
    return exp(-(SQR(x0 - 0.35) + SQR(x1 - 0.15)) / 0.01);
}
// --------------------------------------------------------------------------
        ...
        double *x[], double *y[], Grid_Info g, double tau, fun_t fun)
{
    ...
    {
        i1 = 0;
        idx1l = IDX2(i0, i1, 0, 0, g.Ns0+1, g.Ns1+1);

        i1 = g.Ns1;
        idx1r = IDX2(i0, i1, 0, 0, g.Ns0+1, g.Ns1+1);

        y[mid][idx1r] = y[mid][idx1l];
    }
    */
    ...
    for (i0 = 1; i0 <= g.Ns0-1; ++i0)
    {
        for (i1 = 0; i1 <= g.Ns1; ++i1)
        {
            ...
            idx1l = IDX2(i0, (i1-1+g.Ns1) % g.Ns1, 0, 0, g.Ns0+1, g.Ns1+1);
            idx1r = IDX2(i0, (i1+1)       % g.Ns1, 0, 0, g.Ns0+1, g.Ns1+1);
...
int main()
{
    ...
    Grid_Info g = { 50, 50, 1.0, 1.0, -1, -1 };

    const double T = 5*g.L0;
    const int K = 5*g.Ns0;
    const double tau = T / (2*K);   // TODO: what is stability condition?
